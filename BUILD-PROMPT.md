Build this ENTIRE system from scratch in a single project.
This is a closed AWS environment with NO internet access to external AI APIs.
Use Amazon Bedrock (Claude 3 Sonnet) instead of Google Gemini for ALL AI features.
Use @aws-sdk/client-bedrock-runtime with IAM role authentication (no API keys needed for AI).
All AI prompts, JSON output formats, and the 10-stage pipeline remain identical — only the provider call changes.
Follow the spec EXACTLY. Do NOT skip any section.
Build in this order:
1. package.json and project structure
2. database.js with ALL 21 tables and seed data
3. middleware (auth.js, audit.js)
4. All 8 route files (auth, tests, bugs, integrations, dashboard, ai, project, jira)
5. All 3 service files (ai-service.js with Bedrock, jira-service.js, event-processor.js)
6. public/index.html (full SPA — single file with all pages, modals, and JavaScript)
...
- Everything runs as a single Node.js process with SQLite (sql.js) — zero external dependencies besides AWS SDK
...
| AI | **Amazon Bedrock** — Claude 3 Sonnet via `@aws-sdk/client-bedrock-runtime`, pluggable with fallback to built-in heuristic engine |
...
├── services/
│   ├── ai-service.js            # AI QA Architect Engine (Bedrock + heuristic fallback)
...
1. **Amazon Bedrock** (primary): Uses `@aws-sdk/client-bedrock-runtime` with `BedrockRuntimeClient.invokeModel()` — model: `anthropic.claude-3-sonnet-20240229-v1:0`
2. **Heuristic** (fallback): Built-in 10-stage QA Architect pipeline when Bedrock is unavailable

Provider is selected based on environment variables:
- `AWS_REGION` is set AND Bedrock is accessible → uses Bedrock (Claude 3)
- Otherwise → uses heuristic engine
...
### AI Prompt
The prompt instructs the AI to act as a senior QA Architect with 15+ years experience, covering all 10 stages, outputting structured JSON with Hebrew or English based on input language. Temperature: 0.4, max tokens: 8192. The prompt must request JSON output.
...
Uses Jaccard similarity on tokenized titles/descriptions. With Bedrock: sends existing bug list and new bug for AI comparison with similarity scores. Threshold: 0.25 for heuristic, 0.3 for AI.

## 6.4 Root Cause Analysis

Heuristic: pattern matching on environment clusters, similar component areas, severity distribution. Bedrock: full analysis prompt returning rootCause, suggestedFix, confidence, relatedPatterns.

## 6.5 Risk Prediction

Calculates: recent bug rate (7 days), critical open bugs, suite failure rates. Risk levels: low/medium/high/critical. Bedrock enhances with natural language analysis and recommendations.
...
- Add Integration modal: name, type, URL, token

### AI Lab
- Status banner showing AI provider (Bedrock/Heuristic) and project knowledge status
...
### Project
- Banner explaining knowledge base concept
- **System Documents** (primary): paste text or upload file, document name, type selector, word count. Active documents list with view/delete.
...
- **Checkboxes**: Custom div-based checkboxes with `data-checked` attribute for reliable visual state
...
- `authenticateToken` middleware on all routes except login, register, webhooks
